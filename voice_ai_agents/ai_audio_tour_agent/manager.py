...
class TourError(Exception):
    """Base exception for Tour Manager operations."""
    pass

class AgentInvokeError(TourError):
    """Raised when an agent fails to return the expected output."""
    pass


class TourManager:
    """
    Orchestrates the full flow of generating a multi-agent audio tour.
    It manages the planner, specialist researchers, and the final orchestrator.
    """

    def __init__(self) -> None:
        """Initialize the manager with a console and printer."""
        self.console = Console()
        self.printer = Printer(self.console)
        logger.info("TourManager initialized.")

    async def run(self, query: str, interests: List[str], duration: str) -> str:
        """
        Main entry point to execute the tour research and generation.
        
        Args:
            query (str): The location to research.
            interests (List[str]): List of selected categories.
            duration (str): Desired duration in minutes.
            
        Returns:
            str: The final assembled tour transcript.
        """
        trace_id = gen_trace_id()
        logger.info(f"Starting tour run for {query} (Interests: {interests}, Duration: {duration}) [Trace: {trace_id}]")
        ...
        with trace("Tour Research trace", trace_id=trace_id):
            ...
            try:
                ...
                # Research selected interests sequentially
                ...
                self.printer.update_item("final_report", "Research complete.", is_done=True)
                self.printer.end()
                logger.info("Tour research and assembly completed successfully.")
            except Exception as e:
                logger.error(f"Tour generation failed: {e}")
                raise TourError(f"Failed to generate tour for {query}: {e}")
